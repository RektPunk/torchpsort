...
`torchpsort` provides two simple functions: `soft_sort` and `soft_rank`. They behave exactly like the original `torchsort` API.

```python
import torch
from torchpsort import soft_rank, soft_sort
